...
'''.format(force_output_sql.value, force_output_sql_unitconv.value)
    ...
    def run_build(self, build_tree: BuildTree):
        ...
        for this_entry in self.entries:
            ...
            build_dir: str = build_tree.build_dir
            test_files_dir: str = build_tree.test_files_dir
            base_name: str = this_entry.basename
            weather_dir: str = build_tree.weather_dir

            # first remove the previous test directory for this file and rename it
            test_run_directory: str = os.path.join(build_dir, this_test_dir, base_name)
            ...
            full_input_file_path: str = os.path.join(test_files_dir, this_entry.name_relative_to_testfiles_dir)
            ...
            if full_input_file_path.endswith('.idf'):
                ...
                source_files: list[str] = os.listdir(test_files_dir)
                for file_name in source_files:
                    if file_name[-4:] == '.imf':
                        full_file_name: str = os.path.join(test_files_dir, file_name)
                        shutil.copy(full_file_name, os.path.join(build_dir, this_test_dir, base_name))
            ...
            with io.open(
                    os.path.join(build_tree.build_dir, this_test_dir, base_name, ep_in_filename),
                    'w',
                    encoding='utf-8'
            ) as f_i:
                f_i.write("%s\n" % idf_text)

            rvi: str = os.path.join(test_files_dir, base_name) + '.rvi'
            if os.path.exists(rvi):
                shutil.copy(rvi, os.path.join(build_tree.build_dir, this_test_dir, base_name, 'in.rvi'))

            mvi: str = os.path.join(test_files_dir, base_name) + '.mvi'
            if os.path.exists(mvi):
                shutil.copy(mvi, os.path.join(build_tree.build_dir, this_test_dir, base_name, 'in.mvi'))

            # pick up the corresponding python plugin file, for now this is just the idf basename with .py extension
            py: str = os.path.join(test_files_dir, base_name) + '.py'
            ...
            epw_path: str = os.path.join(weather_dir, self.default_weather_filename)
            if this_entry.epw:
                ...
                if not epw_exists:
                    ...
                    epw_path = os.path.join(weather_dir, self.default_weather_filename)
