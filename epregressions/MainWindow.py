...
# On windows, you'll need the appropriate (architecture, python version) of pywin32:
#   http://sourceforge.net/projects/pywin32/files/pywin32/
# On Mac, maybe growl? (http://stackoverflow.com/questions/10608678/growl-notification-in-python)
# The following lines do the appropriate import trials and sets global flags to be used later
do_win32api_notifications = False
do_pynotify_notifications = False
do_mac_notifications = False
if platform == "linux":
    try:
        import pynotify
...
class PyApp(Gtk.Window):
    ...
    def gui_fill_with_data(self):
        ...
        # num threads here
        if self.suiteargs.force_run_type != None:
    ...
    def handle_treeview_row_activated(self, tv_widget, path_tuple, view_column):
        ...
        (model, path) = self.tree_selection.get_selected()
        ...
        casename = model.get_value(tree_iter, 0)
        # Clean the filename entry
        if ":" in casename:
            colon_index = casename.index(":")
            casename = casename[:colon_index]
        test_dir = "Tests"
    ...
    def idf_selection_extint(self, widget, calltype):
        if not self.idfs_have_been_built:
            self.warning_not_yet_built()
            return
        column = IDFListViewColumnIndex.ExtInt
        selection = False
        if calltype == "select":
            selection = True
        for filea in self.idfliststore:
    ...
    def idf_selection_groundht(self, widget, calltype):
        if not self.idfs_have_been_built:
            self.warning_not_yet_built()
            return
        column = IDFListViewColumnIndex.groundht
        selection = False
        if calltype == "select":
            selection = True
        for filea in self.idfliststore:
    ...
    def idf_selection_dataset(self, widget, calltype):
        if not self.idfs_have_been_built:
            self.warning_not_yet_built()
            return
        column = IDFListViewColumnIndex.dataset
        selection = False
        if calltype == "select":
            selection = True
        for filea in self.idfliststore:
    ...
    def simulationscomplete_callback_handler(self):
        ...
        self.send_notification("Test suite simulations have been completed\nPost-processing beginning now...")
    ...
    def alldone_callback_handler(self, results):
        ...
        for entry in results:
            ...
            if entry.has_eso_diffs:
                ...
                if not entry.basename in totaldifffiles_files:
                ...
                if entry.eso_diffs.count_of_big_diff > 0:
                    ...
                    if not entry.basename in numbigdiffs_files:
                    ...
                    if not entry.basename in numsmalldiffs_files:
        ...
        path = self.resultsliststore.get_path(self.resultsparent_numrun)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_success)
        self.treeView.expand_row(path, False)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_unsuccess)
        self.treeView.expand_row(path, False)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_success2)
        self.treeView.expand_row(path, False)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_bigmath)
        self.treeView.expand_row(path, False)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_smallmath)
        self.treeView.expand_row(path, False)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_bigtable)
        self.treeView.expand_row(path, False)
        ...
        path = self.resultsliststore.get_path(self.resultsparent_textual)
        self.treeView.expand_row(path, False)
        ...
        self.send_notification("Test suite has been completed")
